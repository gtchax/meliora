 ...
 */
if(!class_exists('Tesla_instagram_block')){
	class Tesla_instagram_block extends WP_Widget {

		function __construct() {
			parent::__construct(
				'tesla_instagram_block',
				'['.THEME_PRETTY_NAME.'] Instagram Block',
				array(
					'description' => _x('Instagram feed.', 'dashboard-widgets','suarez'),
				...
				<div class="block-cover">
					<div class="instagram-meta">
						<div class="meta-inner">
							<?php if($title) : ?>
								<span class="meta-title"><?php tt_print($title) ?></span>
							<?php endif; ?>
							<i class="icon-instagram"></i>
							<span class="insta-username">@<?php tt_print($instance['username']) ?></span>
						</div>
					</div>                     
					<a href="https://www.instagram.com/<?php tt_print($instance['username']) ?>" target="_blank">
						<img src="<?php tt_print($instance['background']) ?>" alt="instagram featured" />
					</a>
				</div>
				<?php //call helper function
		...
		function form($instance) {
			$instance = wp_parse_args((array) $instance, array(
				'title'		=> _x('Instagram Block','dashboard-widgets','suarez'),
				'background'	=> get_template_directory_uri().'/assets/instagram-main.jpg',
				'nr_images'	=> 3,
				'username'	=> '',
				'cache_h'	=> 1
				));
			...
			<p><label><?php _ex('Title:','dashboard-widgets','suarez') ?></label>
			...
			<p><label><?php _ex('Background Image:','dashboard-widgets','suarez') ?></label>
				<input type="text" name="<?php echo esc_attr($this->get_field_name( 'background' ))?>" id="<?php echo esc_attr($this->get_field_id( 'background' ))?>" value="<?php echo esc_attr($instance['background'])?>" class="widefat" />
			</p>
			<p><label><?php _ex('Image nr:','dashboard-widgets','suarez') ?></label>
			...
			<p><label><?php _ex('Username:','dashboard-widgets','suarez') ?></label>
			...
			<p><label><?php _ex('Cache Hours:','dashboard-widgets','suarez') ?></label>
	...
	add_action('widgets_init', create_function('', 'return register_widget("Tesla_instagram_block");'));
}